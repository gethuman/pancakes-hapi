...
function addWebRoutes(opts) {
    ...
    server.on('response', function (request) {
        if ( request.response.statusCode > 399 ) {
            ...
            console.log(request.method.toUpperCase() + ' ' + request.info.hostname + request.url.path + ' -> '
...
function getWebRouteHandler(opts) {
    ...
    return function handleWebRoute(request, reply, urlOverride, returnCode) {
        ...
        if (routeInfo.redirect) {
            ...
            return reply().redirect(redirectUrl).permanent(true);
        }
        ...
        try {
            ...
            /* eslint no-console:0 */
            console.log('Error while trying to process request: ' + err.stack);

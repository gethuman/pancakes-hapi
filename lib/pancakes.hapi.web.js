...
function getWebRouteHandler(opts) {
    ...
    return function handleWebRoute(request, reply, urlOverride, returnCode) {
        ...
        var routeInfo = null;
        var referer = request && request.headers && request.headers.referer ? request.headers.referer : '';
        var routeHelper = me.pancakes.cook('routeHelper');
        // var newrelic = require('newrelic');

        Q.fcall(function () {
            routeInfo = webRouteHandler.getRouteInfo(appName, url, request.query, request.app.lang, request.user, referer);

...
function addWebRoutes(opts) {
    ...
    server.on('response', function (request) {
        ...
        var reqInfo = request.info || {};
        var referrer = reqInfo.referrer || '';
        var host = reqInfo.hostname || '';
        var url = request.url || {};
        var urlPath = url.path || '';

        if (statusCode > 399 && method.toUpperCase && host && urlPath) {

            var lapse = -1;
            if (reqInfo.responded && reqInfo.received) {
                lapse = reqInfo.responded - reqInfo.received;
            }

            var msg = method + ' ' + host + urlPath + ' -> ' + statusCode + ' ' + '(' + lapse + 'ms)';
            if (referrer) {
...
function getWebRouteHandler(opts) {
    ...
    return function handleWebRoute(request, reply, urlOverride, returnCode) {
        ...
        var routeInfo = null;
        var referrer = request && request.headers && request.headers.referer ? request.headers.referer : '';
        var routeHelper = me.pancakes.cook('routeHelper');
        // var newrelic = require('newrelic');

        Q.fcall(function () {
            routeInfo = webRouteHandler.getRouteInfo(appName, url, request.query, request.app.lang, request.user, referrer);

...
function addWebRoutes(opts) {
    ...
    server.on('response', function (request) {
        if ( request.response && request.response.statusCode && request.response.statusCode > 399 ) {
            var lapse = request.info.responded - request.info.received;
...
function getWebRouteHandler(opts) {
    ...
    return function handleWebRoute(request, reply, urlOverride, returnCode) {
        ...
        try { // this can throw an error, though not likely now that there's a catch/return above...
        ...
        try {
            webRouteHandler.processWebRequest(routeInfo, callbacks)
                ...
                .catch(function (e) {
                    console.log('Error while trying to process req: ' + e);
                    reply(new Error('Sorry- there is an error here, and we are trying to fix it. Please try again later.'));
                })

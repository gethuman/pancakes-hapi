...
function processRoute(opts) {
    if (routeHandler) {
        routeHandler(opts.request, opts.reply, opts.urlOverride);
    }
    else {
        opts.reply.continue();
    }
...
function getWebRouteHandler(opts) {
    ...
    return function handleWebRoute(request, reply, urlOverride) {
        var appName = request.app.name;
        var url = urlOverride || request.url.pathname;
        var routeInfo = webRouteHandler.getRouteInfo(appName, url, request.query, request.app.lang);
        var routeHelper = me.pancakes.cook('routeHelper');

        var newrelic = require('newrelic');
        newrelic.setTransactionName(routeInfo.appName + '||' + routeInfo.lang + '||' + routeInfo.url);
